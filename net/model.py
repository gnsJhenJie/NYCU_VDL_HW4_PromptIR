...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.qkv_dwconv = nn.Conv2d(
            dim * 3, dim * 3, kernel_size=3, stride=1, padding=1, groups=dim *
            3, bias=bias)
...
class resblock(nn.Module):
    def __init__(self, dim):
        ...
        self.body = nn.Sequential(nn.Conv2d(
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(
                n_feat, n_feat // 2, kernel_size=3, stride=1, padding=1,
                bias=False),
            nn.PixelUnshuffle(2))
...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim=128, prompt_len=5, prompt_size=96,
        ...
        self.conv3x3 = nn.Conv2d(
            prompt_dim, prompt_dim, kernel_size=3, stride=1, padding=1,
            bias=False)
    ...
    def forward(self, x):
        ...
        prompt = prompt_weights.unsqueeze(-1).unsqueeze(-1).unsqueeze(
            -1) * self.prompt_param.unsqueeze(0).repeat(B, 1, 1, 1, 1, 1).squeeze(1)
    ...
    def __init__(self,
        ...
        if self.decoder:
            ...
            self.prompt3 = PromptGenBlock(
                prompt_dim=320, prompt_len=5, prompt_size=8, lin_dim=384)
